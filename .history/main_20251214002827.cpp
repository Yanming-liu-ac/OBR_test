...
bool compare_bids_desc(const std::pair<double, int>& a, const std::pair<double, int>& b) {
    // Make ordering strict even with NaN to avoid UB in std::sort
    const bool a_nan = std::isnan(a.first);
    const bool b_nan = std::isnan(b.first);
    if (a_nan || b_nan) {
        if (a_nan && b_nan) return false;   // equivalent
        return !a_nan && b_nan;             // NaN goes to the end
    }
...
bool compare_asks_asc(const std::pair<double, int>& a, const std::pair<double, int>& b) {
    ...
    if (a_nan || b_nan) {
        if (a_nan && b_nan) return false;   // equivalent
        return b_nan && !a_nan;             // NaN goes to the end
    }
    return a.first < b.first;
}
...
void read_order_file(const std::string& filename, std::vector<Order>& orders) {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            if (fields.size() >= 8) {
                // Trim helper (only spaces/tabs; CSV is assumed unquoted)
                ...
                if (fields[5].empty()) {
                    std::cout << "Warning: Line " << line_num << " has empty ordertype; skip." << std::endl;
                    continue;
                }
                Order order;
                ...
                order.ordertype = fields[5].empty() ? '\0' : fields[5][0];
                order.price = std::atof(fields[6].c_str());
                order.orderqty = std::atoi(fields[7].c_str());

                if (!std::isfinite(order.price)) {
                    std::cout << "Warning: Line " << line_num << " has non-finite price; skip." << std::endl;
                    continue;
                }
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            if (fields.size() >= 10) {
                ...
                if (fields[4].empty()) {
                    std::cout << "Warning: Line " << line_num << " has empty exectype; skip." << std::endl;
                    continue;
                }
                Trade trade;
                ...
                trade.exectype = fields[4].empty() ? '\0' : fields[4][0];
                ...
                if (!std::isfinite(trade.tradeprice) || !std::isfinite(trade.trademoney)) {
                    std::cout << "Warning: Line " << line_num << " has non-finite trade value; skip." << std::endl;
                    continue;
                }
        ...
        if (trades[i].exectype == 'f') {
            for (size_t j = 0; j < orders.size(); j++) {
                if ((orders[j].applseqnum == trades[i].bidapplseqnum || 
                    ...
                    std::llabs(orders[j].transacttime - trades[i].transacttime) <= 1000) {
