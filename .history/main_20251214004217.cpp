...
// Separate book for bid or ask side
struct SideBook {
    std::map<int, BookOrder> orders;  // applseqnum -> BookOrder
    
    // Get best price for this side (high for bid, low for ask)
    double get_best_price(bool is_bid) const {
        if (orders.empty()) return 0;
        
        if (is_bid) {
            // For bids, best = highest
            double best = 0;
            for (std::map<int, BookOrder>::const_iterator it = orders.begin(); 
                 it != orders.end(); ++it) {
    ...
    void add(const BookOrder& order) {
        orders[order.applseqnum] = order;
    }
    
    // Remove order from this side
    void remove(int applseqnum) {
        orders.erase(applseqnum);
    }
    
    // Update order quantity (reduce by traded amount)
    bool update_qty(int applseqnum, int traded_qty) {
        std::map<int, BookOrder>::iterator it = orders.find(applseqnum);
        if (it != orders.end()) {
            it->second.qty -= traded_qty;
            if (it->second.qty <= 0) {
                orders.erase(it);
                return true;  // Order fully filled
            }
...
struct OrderBook {
    SideBook bid_book;  // Separate book for buy orders
    SideBook ask_book;  // Separate book for sell orders
...
// Sort by time first, then put orders before trades if time is the same
bool compare_events(const Event& a, const Event& b) {
    if (a.time != b.time) return a.time < b.time;
    // When times are equal, orders should come before trades
    if (a.type == "order" && b.type == "trade") return true;
    if (a.type == "trade" && b.type == "order") return false;
    return false;  // Both same type
}
...
void process_events(const std::vector<Order>& orders, 
    ...
    // Sort events: by time first, then orders before trades
    ...
    for (size_t i = 0; i < events.size(); i++) {
        if (events[i].type == "order") {
            ...
            // Add order to book (for all orders, including pre-opening)
            add_order(book, order);
            
            // Take snapshot for orders after market opening
            if (order.transacttime >= OPENING_TIME) {
                if (!market_opened) {
            ...
            if (trade.transacttime >= OPENING_TIME) {
